...
CACHED_KANJI_NAME = "DailyKanji_Cached_Item"
CACHED_KANJI_CHARACTER = "DailyKanji_Cached_Image"
...
def main(config):
    ...
    # DISPLAY THE KANJI CHARACTER on half the screen width
    if canvas.is2x():
        display_items.append(add_padding_to_child_element(render.Image(height = image_width, width = image_width, src = kanji_image_src), -11, -27))
    else:
        display_items.append(add_padding_to_child_element(render.Image(height = image_width, width = image_width, src = kanji_image_src), -6, -13))

    display_items.append(add_padding_to_child_element(render.Marquee(width = int(SCREEN_WIDTH / 2), child = render.Text(row1, color = "#65d0e6", font = FONT)), int(SCREEN_WIDTH / 2), 0))
    display_items.append(add_padding_to_child_element(render.Marquee(width = int(SCREEN_WIDTH / 2), child = render.Text(row2, color = "#f4a306", font = FONT)), int(SCREEN_WIDTH / 2), FONT_HEIGHT))
    display_items.append(add_padding_to_child_element(render.Marquee(width = int(SCREEN_WIDTH / 2), child = render.Text(row3, color = "#e77c05", font = FONT)), int(SCREEN_WIDTH / 2), 2 * FONT_HEIGHT))
...
def display_kanji_with_image_url(individual_kanji, kanji_alive_key):
    ...
    for kanjicharacter in individual_kanji:
        i = i + 1
        url = KANJI_API_URL.format(kanjicharacter)
